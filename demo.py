...
file_path = "/Users/leeboyeon/.cache/torch/hub/checkpoints/s3fd-619a316812.pth"

# 파일이 이미 존재하는지 확인
if os.path.exists(file_path):
    print(f"파일 '{file_path}'이 이미 존재합니다.")
else:
    # 파일이 존재하지 않으면 URL에서 다운로드
    try:
        print(f"파일 '{file_path}'을 다운로드 중...")
        download_file(url, file_path)
        print(f"파일 '{file_path}'이 다운로드되었습니다.")
    except Exception as e:
        print(f"파일 다운로드 중 오류가 발생했습니다: {e}")

print(f"모델 파일이 '{file_path}'에 저장되었습니다.")
...
def find_best_frame(source, driving, cpu):
    url2 = "https://www.adrianbulat.com/downloads/python-fan/2DFAN4-cd938726ad.zip"
    file_path2 = "/Users/leeboyeon/.cache/torch/hub/checkpoints/2DFAN4-cd938726ad.zip"
    if os.path.exists(file_path2):
        print(f"프레임 파일 '{file_path2}'이 이미 존재합니다.")
    else:
        # 파일이 존재하지 않으면 URL에서 다운로드
        try:
            print(f"프레임 파일 '{file_path2}'을 다운로드 중... \n 만약 다운로드가 오랜시간 동안 완료되지 않을 경우 수동 다운로드가 필요합니다.")
            download_file(url2, file_path2)
            print(f"프레임 파일이 '{file_path2}' 경로에 다운로드 되었습니다.")
        except Exception as e:
            print(f"프레임 파일 다운로드 중 오류가 발생했습니다: {e}")
